...
FROM exam_tbl exam, question_tbl question, option_tbl options
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if ($row['question_id'] !== $current_question_id) {
        ...
        if ($current_question_id !== null) {
            $correct_answer_query = "SELECT option_title FROM option_tbl WHERE question_id = $current_question_id AND option_number = $correct_answer_option_number ";
...
?>
        ...
        $correct_answer_option_number = $row['answer_option'];
        ?>
