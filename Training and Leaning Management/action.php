 ...
 <?php
    ...
    if (isset($_POST['add_exam'])) {
        ...
        if (
            ...
            $query = "INSERT INTO hr1_exam(job_list_id, exam_title, exam_duration, mark_per_right_answer, mark_per_wrong_answer, exam_status, num_items) 
...
if (isset($_POST['add_questions'])) {
    ...
    $query = "SELECT num_items FROM hr1_exam WHERE id = '$exam_id'";
    $result = mysqli_query($con, $query);
    $row = mysqli_fetch_assoc($result);
    $number_of_items = $row['num_items'];

    mysqli_autocommit($con, false);

    $query1 = "INSERT INTO hr1_question(exam_id, question_title, correct_answer_option) 
    ...
    if (mysqli_query($con, $query1)) {
        ...
        foreach ($options as $option_number => $option_title) {
            $query2 = "INSERT INTO hr1_option (question_id, option_title, option_number) 
            ...
            if (mysqli_query($con, $query2)) {
                ...
                $query = "SELECT COUNT(*) AS count FROM hr1_question WHERE exam_id = '$exam_id'";
                $result = mysqli_query($con, $query);
                $row = mysqli_fetch_assoc($result);
                $count = $row['count'];
                ...
                $query3 = "UPDATE hr1_exam SET exam_status = '$status' WHERE id = '$exam_id'";
